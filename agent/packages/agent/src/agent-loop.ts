...
/** Pull the first user text message (task prompt) from the transcript. */
function extractFirstUserTaskText(messages: AgentMessage[]): string {
...
/**
 * Rough wall-clock budget (seconds) from prompt shape: bullets, length, and optional criteria count
 * from the system prompt. Aligns short tasks with shorter internal nudges / graceful exit.
 * Caps are conservative vs wall-clock runners to reduce outer timeouts.
 */
function estimateBudgetSecondsFromPrompt(taskText: string, criteriaHint: number): number {
	const trimmed = taskText.trim();
	if (!trimmed) {
		return 240;
	}
	const bulletMatches = trimmed.match(/^\s*[-*•]\s/gm)?.length ?? 0;
	const words = trimmed.split(/\s+/).filter(Boolean).length;
	const lines = trimmed.split("\n").length;
	const criteriaCount = Math.max(criteriaHint > 0 ? criteriaHint : 0, bulletMatches);
	const bulletScore = Math.min(1, criteriaCount / 12);
	const wordScore = Math.min(1, words / 6000);
	const lineScore = Math.min(1, lines / 100);
	const complexity = Math.min(1, Math.max(bulletScore, wordScore * 0.65 + lineScore * 0.35));
	return Math.round(Math.min(240, Math.max(75, 38 + complexity * 202)));
}

const DEFAULT_GRACEFUL_EXIT_MS = 290_000;

interface DerivedLoopTiming {
	EARLY_NUDGE_MS: number;
	URGENT_NUDGE_MS: number;
	LATE_NUDGE_MS: number;
	GRACEFUL_EXIT_MS: number;
	PREEMPT_EXIT_MS: number;
	MID_COVERAGE_NUDGE_MS: number;
	MID_COVERAGE_MAX_ELAPSED_MS: number;
	FORCE_EDIT_MS: number;
	SPARSE_EDIT_NUDGE_MS: number;
	REVIEW_PASS_MAX_ELAPSED_MS: number;
	SLOW_PACE_MIN_ELAPSED_MS: number;
	SLOW_PACE_REPEAT_MS: number;
}

function deriveLoopTimingFromBudgetSec(budgetSec: number): DerivedLoopTiming {
	const clampedSec = Math.min(240, Math.max(60, budgetSec));
	const gracefulExitMs = Math.max(45_000, clampedSec * 1000 - 30_000);
	const preemptExitMs = Math.floor((gracefulExitMs * 200) / 290);
	const scale = gracefulExitMs / DEFAULT_GRACEFUL_EXIT_MS;
	const tailMargin = Math.max(18_000, Math.round(32_000 * scale));
	const midCoverageNudgeMs = Math.max(25_000, Math.round(50_000 * scale));
	return {
		EARLY_NUDGE_MS: Math.max(4_000, Math.round(9_000 * scale)),
		URGENT_NUDGE_MS: Math.max(10_000, Math.round(19_000 * scale)),
		LATE_NUDGE_MS: Math.max(22_000, Math.round(48_000 * scale)),
		GRACEFUL_EXIT_MS: gracefulExitMs,
		PREEMPT_EXIT_MS: preemptExitMs,
		MID_COVERAGE_NUDGE_MS: midCoverageNudgeMs,
		MID_COVERAGE_MAX_ELAPSED_MS: Math.max(midCoverageNudgeMs + 5_000, gracefulExitMs - tailMargin),
		FORCE_EDIT_MS: Math.max(16_000, Math.round(38_000 * scale)),
		SPARSE_EDIT_NUDGE_MS: Math.max(12_000, Math.round(26_000 * scale)),
		REVIEW_PASS_MAX_ELAPSED_MS: Math.max(24_000, Math.round(50_000 * scale)),
		SLOW_PACE_MIN_ELAPSED_MS: Math.max(8_000, Math.round(16_000 * scale)),
		SLOW_PACE_REPEAT_MS: Math.max(12_000, Math.round(24_000 * scale)),
	};
}
...
async function runLoop(
	...
	const taskPromptText =
		extractFirstUserTaskText(newMessages) || extractFirstUserTaskText(currentContext.messages);
	const estimatedBudgetSec = estimateBudgetSecondsFromPrompt(taskPromptText, expectedCriteriaCount);
	const {
		EARLY_NUDGE_MS,
		URGENT_NUDGE_MS,
		LATE_NUDGE_MS,
		GRACEFUL_EXIT_MS,
		PREEMPT_EXIT_MS,
		MID_COVERAGE_NUDGE_MS,
		MID_COVERAGE_MAX_ELAPSED_MS,
		FORCE_EDIT_MS,
		SPARSE_EDIT_NUDGE_MS,
		REVIEW_PASS_MAX_ELAPSED_MS,
		SLOW_PACE_MIN_ELAPSED_MS,
		SLOW_PACE_REPEAT_MS,
	} = deriveLoopTimingFromBudgetSec(estimatedBudgetSec);
	const CONSECUTIVE_EDIT_THRESHOLD = 18;
	...
	const recordSuccessfulFileMutation = async (targetPath: string): Promise<void> => {
		...
		let breadthHint = "";
		if (consecutiveEditsOnSameFile >= CONSECUTIVE_EDIT_THRESHOLD && uneditedTargets.length > 0) {
	...
	while (true) {
		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			...
			// v217: Slow-pace detection — ~8s/turn threshold, repeating on SLOW_PACE_REPEAT_MS
			if (totalLlmRequests >= 3 && pendingMessages.length === 0) {
				const elapsed = Date.now() - loopStart;
				const avgPace = elapsed / totalLlmRequests;
				if (avgPace > 8_000 && elapsed > SLOW_PACE_MIN_ELAPSED_MS && (Date.now() - lastSlowPaceNudgeAt) > SLOW_PACE_REPEAT_MS) {
			...
			if (hasProducedEdit && pendingMessages.length === 0) {
				...
				if (elapsed >= MID_COVERAGE_NUDGE_MS && editedPaths.size < foundFiles.length && uneditedTop.length > 0 && elapsed < MID_COVERAGE_MAX_ELAPSED_MS) {
			...
			if (hasMoreToolCalls) {
				...
				// FORCE EDIT: if past FORCE_EDIT_MS with no edit and we have read files, demand edit NOW
				// Clears pending messages to ensure this always triggers
				if (!hasProducedEdit && (Date.now() - loopStart) >= FORCE_EDIT_MS && pathsAlreadyRead.size > 0) {
					const topFile = foundFiles[0] || [...pathsAlreadyRead][0] || "";
					if (topFile) {
						pendingMessages = [{
							role: "user",
				...
				if (hasProducedEdit && pendingMessages.length === 0) {
					...
					if (uneditedFound.length > 0 && elapsed > SPARSE_EDIT_NUDGE_MS && uniqueEdited.size <= 2) {
						// by ryc
						...
						pendingMessages.push({
							...
							content: [{
								type: "text",
								text: `${Math.round(SPARSE_EDIT_NUDGE_MS / 1000)}s+ elapsed and you have only edited ${uniqueEdited.size} file(s). ${uneditedFound.length} discovered target(s) remain: ${uneditedFound.slice(0, 8).map((f: string) => `\`${f}\``).join(", ")}. Read and edit each one before going back to files you already edited.`,
							}],
							timestamp: Date.now(),
						});
					}
				...
				if (
					...
					pendingMessages.push({
						...
						content: [
							{
								type: "text",
								text: `Over ${Math.round(LATE_NUDGE_MS / 1000)}s without successful file changes. Pick the clearest path from the task or keyword list and apply \`edit\` or \`write\` now — further discovery has diminishing returns.`,
							},
						],
						timestamp: Date.now(),
					});
		...
		if (!reviewPassDone && hasProducedEdit && reviewElapsed < REVIEW_PASS_MAX_ELAPSED_MS) {
			reviewPassDone = true;
			workPhase = "search";
