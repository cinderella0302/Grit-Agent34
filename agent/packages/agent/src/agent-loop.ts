...
async function runLoop(
	...
	// v246: pre-fetch + on-disk gutting + user-prompt nudges removed.
	// All target-file guidance now flows through the system prompt's
	// "Reference target files (MANDATORY)" section, which `runReferenceExploit`
	// populates via `setReferenceTargets` before this loop starts.
	...
	while (true) {
		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			...
			// v246: removed v217 slow-pace and v222 mid-run coverage user-prompt
			// nudges. The system-prompt mandate is the single source of guidance.
