...
export function createPlanToolDefinition(): ToolDefinition<typeof planSchema, PlanToolDetails> {
	return {
		...
		promptGuidelines: [
			...
			"If a validated `plan` attempt fails, do not immediately re-submit near-identical JSON. First identify each failed plan item and materially rewrite the failing sections/bullets/paths before calling `plan` again.",
			...
			"Before each `plan` call, run an internal preflight checklist for every item: criterion coverage, concrete Edits bullets, exhaustive Readrequired with already-read paths, and dependency-safe ordering. If any item fails preflight, do not call `plan` yet.",
